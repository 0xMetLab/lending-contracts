...
contract BountyManager is Initializable, OwnableUpgradeable, PausableUpgradeable, RecoverERC20 {
	 ...
	 * @param _mfd MFD, to send bounties as vesting Met to Hunter (user calling bounty)
	 * @param _chef CIC, to query bounties for ineligible emissions
	 * @param _priceProvider PriceProvider service, to get Met price for bounty quotes
	 ...
	 * @param _baseBountyUsdTarget Base Bounty is paid in Met, will scale to match this USD target value
	...
	function initialize(
		...
		if (_met == address(0)) revert AddressZero();
	 ...
	 * @param _user address
	 * @return bounty amount of Met Hunter will recieve.
	 ...
	 * @return bounty in Met to be paid to Hunter (via vesting)
	...
	function executeBounty(
		...
		if (_execute && bounty != 0) {
			if (!issueBaseBounty) {
				IERC20(met).safeTransferFrom(incentivizer, address(this), totalBounty);
			}
	 ...
	 * @return totalBounty raw amount of Met returned from Incentivizer. Hunter % will be deducted from this.
	 * @return issueBaseBounty whether Incentivizer will pay bounty from its own Met reserve, or from this contracts Met reserve
	 ...
	 * @return totalBounty Met to pay for this _user's bounty execution
	 ...
	 * @return totalBounty Met to pay for this _user's bounty execution. paid from Autocompound fee
	...
	/**
	 * @notice Return Met amount for Base Bounty.
	 * Base Bounty used to incentivize operations that don't generate their own reward to pay to Hunter.
	 * @return bounty in Met
	 */
	function getBaseBounty() public view whenNotPaused returns (uint256) {
		uint256 metPrice = IPriceProvider(priceProvider).getTokenPriceUsd();
		uint256 bounty = (baseBountyUsdTarget * 1e8) / metPrice;
