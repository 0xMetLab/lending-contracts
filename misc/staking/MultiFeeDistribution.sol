  ...
  RecoverERC20
{
  ...
  /// @notice Duration of vesting Met
  ...
  /**
   * @dev Initializer
   *  First reward MUST be the Met token or things will break
   *  related to the 50% penalty and distribution to locked balances.
   * @param metToken_ Met token address
  ...
  function initialize(
    ...
    if (metToken_ == address(0)) revert AddressZero();
    ...
    metToken = IERC20(metToken_);
    daoTreasury = dao_;
    _priceProvider = priceProvider_;
    rewardTokens.push(metToken_);
    rewardData[metToken_].lastUpdateTime = block.timestamp;
   ...
   */
  function setRewardConverter(address rewardConverter_) external onlyOwner {
  ...
  function vestTokens(address user, uint256 amount, bool withPenalty) external whenNotPaused {
    ...
    if (user == address(this)) {
      ...
      _notifyReward(address(metToken), amount);
      return;
    }
    ...
    if (withPenalty) {
      ...
      if (earnings.length > 0 && (earnings[lastIndex].unlockTime / 1 days) == currentDay + vestingDurationDays) {
        ...
        earnings.push(LockedBalance({amount: amount, unlockTime: unlockTime, multiplier: 1, duration: vestDuration}));
      }
    } else {
  ...
  function withdraw(uint256 amount) external {
    ...
    if (amount <= bal.unlocked) {
      ...
      for (i = 0; ; ) {
        ...
        (uint256 withdrawAmount, uint256 penaltyFactor, uint256 newPenaltyAmount, uint256 newBurnAmount) = _penaltyInfo(
          _userEarnings[_address][i]
        );
  ...
  /**
   * @notice Zap vesting Met tokens to LP
   * @param user address
  ...
  function zapVestingToLp(address user) external returns (uint256 zapped) {
    ...
    metToken.safeTransfer(lockZap, zapped);
  ...
  function totalBalance(address user) external view returns (uint256) {
    if (stakingToken == address(metToken)) {
  ...
  function _stake(uint256 amount, address onBehalfOf, uint256 typeIndex, bool isRelock) internal whenNotPaused {
    ...
    for (uint256 i; i < userLocksLength; ) {
      if (userLocks[i].unlockTime / AGGREGATION_EPOCH == unlockWeek && userLocks[i].multiplier == rewardMultiplier) {
   ...
   * @dev for rewards other than Met token, every 24 hours we check if new
  ...
  function _withdrawTokens(
    ...
    metToken.safeTransfer(onBehalfOf, amount);
    if (penaltyAmount > 0) {
      if (burnAmount > 0) {
        metToken.safeTransfer(starfleetTreasury, burnAmount);
      }
      metToken.safeTransfer(daoTreasury, penaltyAmount - burnAmount);
    }
  ...
  function _withdrawExpiredLocksFor(
    ...
    if (isRelockAction && address_ != msg.sender && lockZap != msg.sender) revert InsufficientPermission();
    ...
    if (isRelockAction || (address_ != msg.sender && !autoRelockDisabled[address_])) {
      ...
      if (doTransfer) {
        ...
        emit Withdrawn(address_, amount, _balances[address_].locked, 0, 0, stakingToken != address(metToken));
      } else {
