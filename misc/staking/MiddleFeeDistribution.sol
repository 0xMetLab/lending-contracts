...
contract MiddleFeeDistribution is IMiddleFeeDistribution, Initializable, OwnableUpgradeable, RecoverERC20 {
	...
	/// @notice Fee distributor contract for earnings and Met lockings
	...
	function initialize(
		...
		if (address(metToken_) == address(0)) revert ZeroAddress();
	...
	function forwardReward(address[] memory _rewardTokens) external {
		...
		for (uint256 i = 0; i < length; ) {
			...
			if (rewardToken == address(metToken)) {
	...
	/**
	 * @notice Returns Met token address.
	 * @return Met token address
	 */
	function getMetTokenAddress() external view returns (address) {
		return address(metToken);
	}
