...
contract LendingPool is VersionedInitializable, ILendingPool, LendingPoolStorage {
  ...
  mapping(address => EnumerableSet.AddressSet) internal withdrawDelegates;
  ...
  function withdraw(address asset, uint256 amount, address to) external whenNotPaused returns (uint256) {
    return _withdraw(asset, amount, to, msg.sender);
  }

  function withdraw(address asset, uint256 amount, address to, address onBehalfOf) external returns (uint256) {
    require(withdrawDelegates[onBehalfOf].contains(msg.sender), 'CallerNotDelegatee');
    return _withdraw(asset, amount, to, onBehalfOf);
  }

  /**
   * @notice Delegate withdraws to another address
   * @param delegatee The address to delegate the withdraw to
   */
  function delegateWithdraw(address delegatee) external {
    withdrawDelegates[msg.sender].add(delegatee);
  }

  /**
   * @notice Undelegate withdraws to another address
   * @param delegatee The address to delegate the withdraw to
   */
  function undelegateWithdraw(address delegatee) external {
    withdrawDelegates[msg.sender].remove(delegatee);
  }

  /**
   * @notice Get the addresses of the delegates of a user
   * @param user The address of the user to query
   */
  function withdrawDelegatesOf(address user) external view returns (address[] memory) {
    return withdrawDelegates[user].values();
  }

  function _withdraw(address asset, uint256 amount, address to, address onBehalfOf) internal returns (uint256) {
    DataTypes.ReserveData storage reserve = _reserves[asset];

    address aToken = reserve.aTokenAddress;

    uint256 userBalance = IAToken(aToken).balanceOf(onBehalfOf);
    ...
    IAToken(aToken).burn(onBehalfOf, to, amountToWithdraw, reserve.liquidityIndex);

    emit Withdraw(asset, onBehalfOf, to, amountToWithdraw);
